...
# Modified FSPL formula that returns distance in meters
def calculateDistance(signalLevelInDbm, freqInMHz):
    exp = (27.55 - (20 * math.log10(freqInMHz)) + abs(signalLevelInDbm)) / 20.0;
    return pow(10.0, exp)

# Method used to perform a scan of all networks within range of device
def scan_APs():
    ...
    try:
        ...
        for iface in ifaces:
            ...
            pAvailableNetworkList = pointer(WLAN_BSS_LIST())
            ...
            try:
                ...
                networks = customresize(avail_net_list.NetworkBSS,
                                         avail_net_list.NumberOfItems)

                network_info = []           # Stores {BSSID, distance, RSSI, SSID} tuples
                for network in networks:
                    # For each network (AP), extract necessary information
                    SSID = network.dot11Ssid.SSID[:network.dot11Ssid.SSIDLength].decode()
                    BSSID = ':'.join('%02x' % b for b in network.dot11Bssid).upper()
                    RSSI = network.lRssi
                    distance = calculateDistance(RSSI, 2412)
                    
                    # Store network info in tuple
                    info = [BSSID, distance, RSSI, SSID]
                    if "Secure" in SSID:            # Store only WC-Secure Wi-Fi AP info
